...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `KiemTraDangNhap`(in tdn VARCHAR(45),in mk VARCHAR(45))
BEGIN
declare _iduser_login int;
     DECLARE doesItExist int;

     if((SELECT count(ma_chuc_vu) FROM nhan_vien where ten_dang_nhap=tdn AND mat_khau=mk)>0) then
		SELECT ma_chuc_vu FROM nhan_vien where ten_dang_nhap=tdn AND mat_khau=mk;
     else
		select 0 as ma_chuc_vu;
     end if;
...
-- Dump completed on 2019-01-07  2:38:27
